...
import { AuthTokens, LoginCredentials, User, WebResponse } from '@/lib/types/auth.type';

export const authService = {
  // 1. Login - POST /auth/login
  // Backend returns WebResponse<{ accessToken, refreshToken }>
  login: async (credentials: LoginCredentials): Promise<AuthTokens> => {
    const response = await api.post<WebResponse<AuthTokens>>('/auth/login', credentials);
