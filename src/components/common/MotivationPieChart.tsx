...
const DEFAULT_COLORS = ["#5841EA", "#10b981", "#f59e0b", "#f43f5e", "#38bdf8"];
...
export default function MotivationPieChart({
  ...
  const normalizedData = data.map((item, index) => ({
    category: item.category ?? item.name ?? `Kategori ${index + 1}`,
    value: item.value,
    fill: item.fill ?? DEFAULT_COLORS[index % DEFAULT_COLORS.length],
  }));

  const chartConfig = normalizedData.reduce((acc, item) => {
    acc[item.category] = {
      label: item.category,
      color: item.fill,
    };
